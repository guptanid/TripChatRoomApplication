...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    public void onStart() {
        ...
        // mDatabaseRef.addListenerForSingleValueEvent(dbRefEventListener);
    }
    ...
    public void onStop() {
        ...
        mDatabaseRef.removeEventListener(dbRefEventListener);
    }
        ...
        dbRefEventListener = new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.child("Users").getChildren()) {
                    databaseUsers.add(snapshot.getValue(User.class));
                }
                ChooseIntent();
            }
    ...
    private void ChooseIntent() {
        ...
        if (firebaseUser != null) {
            ...
            if (databaseUsers != null && databaseUsers.size() > 0) {
                for (User user : databaseUsers) {
                    if (user.getUserId() == firebaseUser.getUid()) {
                        loggedInUser = user;
                        break;
                    }
                }
